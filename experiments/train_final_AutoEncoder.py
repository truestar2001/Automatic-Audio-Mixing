...
def run(rank, n_gpus, hps):
  ...
  optim_g = torch.optim.AdamW(
      ...
      eps=hps.train.eps)
  model_path = "/home/yoon/AudioMixing/ckpt/AutoEncoder/step87000.pth"  # 모델 weight 파일 경로
  global_step = 0
  try:
...
def evaluate(hps, generator, eval_loader, writer_eval, criterion, optimizer, best_loss=float('inf')):
    ...
    with torch.no_grad():
        for batch_idx, (X, Y, _) in enumerate(tqdm(eval_loader)):
            ...
            with autocast(device_type='cuda', enabled=False):
                loss = utils.spec_l1_loss(Y, Y_hat, criterion)
    ...
    if average_loss < best_loss:  # 평균 손실이 이전 최저 손실보다 낮으면 저장
        ...
        save_model_state(generator,optimizer,global_step, os.path.join('/home/yoon/AudioMixing/ckpt/AutoEncoder', f'step{global_step}.pth'))  # 모델 저장 경로는 hps에서 가져온다고 가정
    ...
    for batch_idx, (X, Y, _) in enumerate(train_loader):
        ...
        with autocast(device_type='cuda', enabled=False):
          #%
            ...
            spec_l1_loss = utils.spec_l1_loss(Y, Y_hat, criterion)
            loss = energy_l1_loss + spec_l1_loss
          #%
            running_energy_l1_loss += energy_l1_loss.item()
            running_spec_l1_loss += spec_l1_loss.item()
        ...
        if rank == 0 and global_step % hps.train.log_interval == 0:
            ...
            if global_step % 100 == 0 and global_step != 0:
                ...
                avg_spec_l1_loss = running_spec_l1_loss / 100
                avg_loss = running_loss / 100
                print(f'Step {global_step}, energy l1 loss: {avg_energy_l1_loss}, spec l1 loss: {avg_spec_l1_loss},')
                running_energy_l1_loss = 0.0
                running_spec_l1_loss = 0.0
                running_loss = 0.0
