...
def evaluate(hps, generator, eval_loader, writer_eval, criterion, optimizer, best_loss=float('inf')):
    ...
    if average_loss < best_loss:  # 평균 손실이 이전 최저 손실보다 낮으면 저장
        ...
        save_model_state(generator,optimizer,global_step, os.path.join('/home/yoon/AudioMixing/ckpt/WavUNet_final_mss', f'step{global_step}.pth'))  # 모델 저장 경로는 hps에서 가져온다고 가정
